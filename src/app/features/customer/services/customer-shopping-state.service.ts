...
export class CustomerShoppingStateService {
      ...
      if (userId) {
        ...
        if (guestItems.length) {
          for (const guest of await this.carts.productsForGuest(guestItems)) {
            ...
            await this.carts.upsertItem(this.cartId, guest.product.id, quantity);
            merged.set(guest.product.id, { ...guest, id: existing?.id, quantity });
          }
  ...
  async addToCart(product: CustomerProduct, requestedQuantity = 1): Promise<void> {
    ...
    try {
      if (this.cartId) await this.carts.upsertItem(this.cartId, product.id, quantity);
      this.cart.update((lines) =>
        existing
          ? lines.map((line) => (line.product.id === product.id ? { ...line, quantity } : line))
  ...
  async setQuantity(productId: string, requested: number): Promise<void> {
    ...
    const quantity = Math.max(1, Math.min(line.product.stock, requested));
    if (quantity === line.quantity) return;
    this.setPending(productId, true);
    try {
      if (this.cartId) await this.carts.upsertItem(this.cartId, productId, quantity);
      this.cart.update((lines) =>
        lines.map((item) => (item.product.id === productId ? { ...item, quantity } : item)),
      );
      this.persistGuest();
    } catch (error) {
